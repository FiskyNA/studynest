    ...
    <div className="h-full flex flex-col">
      <div className="border-b border-gray-200 bg-white px-6 py-4">
        <div className="flex items-center justify-between mb-4">
          <div><h1 className="text-2xl font-bold">Grades</h1><p className="text-sm text-gray-500">{grades.length} grades</p></div>
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="bg-brand-50 rounded-xl p-4"><p className="text-sm text-brand-600 font-medium">GPA</p><p className="text-3xl font-bold text-brand-700">{overallGPA.toFixed(2)}</p></div>
          <div className="bg-green-50 rounded-xl p-4"><p className="text-sm text-green-600 font-medium">Average</p><p className="text-3xl font-bold text-green-700">{overallAvg.toFixed(1)}% <span className="text-lg">{getLetter(overallAvg)}</span></p></div>
          <div className="bg-purple-50 rounded-xl p-4"><p className="text-sm text-purple-600 font-medium">Subjects</p><p className="text-3xl font-bold text-purple-700">{subjects.length}</p></div>
        </div>
      </div>
      <div className="flex-1 overflow-y-auto p-6">
        {showNew && (
          <div className="bg-white border border-gray-200 rounded-xl p-4 mb-4">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
              <input autoFocus type="text" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="Assignment" className="px-3 py-2 border border-gray-200 rounded-lg text-sm outline-none" />
              <input type="text" value={form.subject} onChange={(e) => setForm({ ...form, subject: e.target.value })} placeholder="Subject" className="px-3 py-2 border border-gray-200 rounded-lg text-sm outline-none" />
              <div className="flex gap-2"><input type="number" value={form.score} onChange={(e) => setForm({ ...form, score: e.target.value })} placeholder="Score" className="flex-1 px-3 py-2 border border-gray-200 rounded-lg text-sm outline-none" /><span className="flex items-center text-gray-400">/</span><input type="number" value={form.max_score} onChange={(e) => setForm({ ...form, max_score: e.target.value })} className="w-16 px-3 py-2 border border-gray-200 rounded-lg text-sm outline-none" /></div>
        ...
        : grades.length === 0 ? <div className="flex flex-col items-center justify-center h-64"><GraduationCap className="w-12 h-12 text-gray-300 mb-4" /><h3 className="font-medium">No grades yet</h3></div>
        : (
          <div className="space-y-6">
            {subjAvg.length > 0 && (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                {subjAvg.map((s) => (<div key={s.subject} className="bg-white border border-gray-200 rounded-xl p-4"><div className="flex items-center justify-between"><h3 className="font-medium">{s.subject}</h3><span className="text-2xl font-bold">{s.grade}</span></div><p className="text-sm text-gray-500">{s.average.toFixed(1)}% average</p></div>))}
              </div>
            )}
            <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
              <table className="w-full">
                <thead><tr className="border-b border-gray-100"><th className="text-left px-4 py-3 text-sm font-medium text-gray-500">Name</th><th className="text-left px-4 py-3 text-sm font-medium text-gray-500">Subject</th><th className="text-left px-4 py-3 text-sm font-medium text-gray-500">Score</th><th className="text-left px-4 py-3 text-sm font-medium text-gray-500">Grade</th><th className="px-4 py-3" /></tr></thead>
                <tbody>{grades.map((g) => { const pct = (g.score / g.max_score) * 100; return (
                  <tr key={g.id} className="border-b border-gray-50 hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-sm">{g.score}/{g.max_score} ({pct.toFixed(0)}%)</td>
                    <td className="px-4 py-3"><span className={clsx('px-2 py-1 rounded-full text-xs font-bold', pct >= 90 ? 'bg-green-100 text-green-700' : pct >= 80 ? 'bg-blue-100 text-blue-700' : pct >= 70 ? 'bg-yellow-100 text-yellow-700' : 'bg-red-100 text-red-700')}>{getLetter(pct)}</span></td>
